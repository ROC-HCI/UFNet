...
YOUTUBEPD_FEATURE_FILE = os.path.join(BASE_DIR,"data/quick_brown_fox/YoutubePD_WavLM.csv")
...
def load(drop_correlated = True, corr_thr = 0.85):
    ...
    labels = 1.0*(df["pd"]!='n')
    print(f"Number of positive and negative samples: {(int)(np.sum(labels))}, {(int)(len(labels)-np.sum(labels))}")
    # df["id"] = df.Filename.apply(parse_patient_id)
    # take string of df["id"] 
