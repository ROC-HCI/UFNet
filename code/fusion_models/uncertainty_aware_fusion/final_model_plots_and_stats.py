...
#2. Load seeds from the wandb experiments
...
class TensorDataset(Dataset):
    def __init__(self,df):
        ...
        self.ids = np.asarray(df["id"]).astype(str)

    def __getitem__(self, index):
        features = []
        for i in range(NUM_MODELS):
            features.append(self.features[i][index])
        return features, self.labels[index], self.ids[index]
...
def evaluate_fusion_model(fusion_model, dataloader, prediction_models, config, split="dev"):
    ...
    #     all_ids = all_ids[~uncertain_indices]
    ...
    if split=="test":
        ...
        seed = config["seed"]
        saved_outputs = {"all_preds":np.asarray(all_final_predictions).flatten(), "all_labels":np.asarray(all_labels), "all_ids":np.asarray(all_ids)}
        with open(f'outputs/preds_with_ids_{seed}_no_drop.pkl', 'wb') as f:
...
def main(**cfg):
    ...
    features, label, pid = train_dataset[0]
    ...
    # fig.write_image("auroc_curve_with_ci_plotly.png")
